...
function addTab(title, url) {
	if ($('#tabsTtoC').tabs('exists', title)) {
		$('#tabsTtoC').tabs('select', title); // 选中并刷新
		var currTab = $('#tabsTtoC').tabs('getSelected');
		// var url = $(currTab.panel('options').content).attr('src');
		if (url != undefined && currTab.panel('options').title != '主页') {
			$('#tabsTtoC').tabs('updateIframeTab', {
...
function tabCloseEven() {
	...
	$('#mm-tabclose').click(function() {
		...
		$('#tabsTtoC').tabs('close', currtab_title);
	});
	// 全部关闭
